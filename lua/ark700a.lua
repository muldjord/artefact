...
function classify(filename)
  _loadUtils()
  printClassifierMessage("Running NIDEK ARK-700A classifier...\n")
  local data = loadDataFromFile(filename)
  return _classify(data) 
end

function _checkCompanyAndModel(data)
  local _, _, company, model = string.find(data, "ID(%a*)/(%a*-%d%d%dA)")

  if (company ~= "NIDEK") or (model ~= "ARK-700A") then
    invalidData("Not NIDEK ARK-700A data.\n")
    return false
  else
...
function interpret(filename, requestedType)
  _loadUtils()
  printInterpreterMessage("Running NIDEK ARK-700A interpreter...\n")
...
function _interpretDKM(data)
  ...
  local root = beginGroup(0, "ark700a_keratometer")
...
function _interpretDRM(data)
  ...
  local root = beginGroup(0, "ark700a_refractometer")
...
function _interpret(data, requestedType)
  ...
  if requestedType == "ark700a_refractometer" then
    _interpretDRM(data)
  elseif requestedType == "ark700a_keratometer" then
    _interpretDKM(data)
  else
...
function _containsDRM(data)
  ...
  drm_block = cut(data, { "DRM" }, { "DKM", "$" })

  if not drm_block then return false end

  return string.find(drm_block, "OR") or string.find(drm_block, "OL")
end

function _containsDKM(data)
  if not data then return false end

  dkm_block = cut(data, { "DKM" }, {"$" })
...
function test()
  ...
  -- Testing classifier
  testReset()
  local data = "DRM  21314VI as as IDNIDEK/ARK-700A"
  TEST_TRUE(_checkCompanyAndModel(data), "Testing _checkCompanyAndModel on good ark-700a data.\n")
  ...
  TEST_FALSE(_checkCompanyAndModel(data), "Testing _checkCompanyAndModel on bad ark-700a data.\n")
...
--  TEST_EQUAL_INT(testInvalidDataCount_, "1", "Testing that _checkChecksum on data with wrong checksum calls invalidData.\n")
  ...
  testReset()
  _interpret( data, "ark700a_refractometer")
--  print("Resume: " .. resume_text)
  ...
  local data = "\001DRM\002BLaBLABLALBLASLDSK\023BLABLAL12.56-00.55\023OL-99.09 20.001799\023\001DKM\002 L07.9507.7117607.83\023DL42.4543.7717643.11-01.32\023 R07.8607.5317507.70\023 R17.5511.3313455.81\023\004"
