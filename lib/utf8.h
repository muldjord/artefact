...
namespace Pentominos {
   ...
   */
  class UTF8CreateException: public Pentominos::Exception {
    public:
    UTF8CreateException(std::string reason) : 
      Pentominos::Exception("Error during creation of the UTF8 subsystem: " + reason) {}
  };

  /**
   * This exception is thrown by UTF8 when the subsystem fails encode the gives string.
   */
  class UTF8EncodeException: public Pentominos::Exception {
    public:
    UTF8EncodeException(std::string reason) : 
      Pentominos::Exception("Error during UTF8 encoding: " + reason) {}
  };

  /**
   * This exception is thrown by UTF8 when the subsystem fails decode the gives string.
   */
  class UTF8DecodeException: public Pentominos::Exception {
    public:
    UTF8DecodeException(std::string reason) : 
      Pentominos::Exception("Error during UTF8 decoding: " + reason) {}
  };
